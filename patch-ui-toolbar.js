...
// 2. Modify fetchYouTubeVideos
const fetchYoutubeRegex = /const fetchYouTubeVideos = async \(\) => \{[\s\S]*?finally \{\s*setLoadingYouTube\(false\);\s*\}\s*\};/g;
const newFetchYoutube = `const fetchYouTubeVideos = async (options?: { preserveMessage?: boolean }) => {
    try {
...
content = content.replace(fetchYoutubeRegex, newFetchYoutube);

// 3. Modify importSelectedFromYouTube to preserve message
const importSelectedRegex = /setYoutubeMessage\(formatVerificationMessage\(data, ids\.length\)\);\s*await loadReleases\(\);\s*await fetchYouTubeVideos\(\);/g;
const newImportSelected = `await loadReleases();
      await fetchYouTubeVideos({ preserveMessage: true });
      setYoutubeMessage(formatVerificationMessage(data, ids.length));
      setSelectedVideoIds(new Set());
      setResolutions({});`;
content = content.replace(importSelectedRegex, newImportSelected);

// 4. Implement toolbar computed properties in the component body
content = content.replace(/const selectAllUnsaved = \(\) => \{[\s\S]*?const clearSelectedVideos = \(\) => \{\s*setSelectedVideoIds\(new Set\(\)\);\s*\};/g, 
`  // Computed Reconcilation
  ...
  const forceResyncAll = () => {
    if (confirm("Re-fetch current YouTube packaging for all " + youtubeCount + " records while preserving governed CMS fields?")) {
      ...
      // Optionally prepopulate resolutions to 'cms' or 'youtube' if there were updates, 
      // but they can do standard import. 
    }
  };`);

// 5. Replace YouTube panel header and toolbar UI
const headerRegex = /<h2 className="text-lg font-semibold" style=\{\{ color: 'var\(--dash-text-primary\)' \}\}>YouTube Video Picker<\/h2>[\s\S]*?<\/div>[\s\S]*?<div className="flex items-center gap-2">[\s\S]*?<\/div>/;

const newHeader = `<div className="w-full">
                ...
                {youtubeCount > 0 && (
                  <div className="bg-neutral-900 border border-neutral-800 rounded-lg p-4 mb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-neutral-800">
                      ...
                      >
                        Advanced: Force Re-sync All ({youtubeCount})
                      </button>
