...
const newNames = ['Omar Hayes', 'Mariam Bennett', 'Michael Rhodes', 'Ayesha Noor', 'Arjun Mehta', 'Raghav Sharma'];
let addedVoc = 0; let updatedVoc = 0;
...
const ytId = 'aMzdiIuYgK4';
const newTitle = 'Take Control | A Ramadan Prayer | SufiPulse USA';
const voc = 'Omar Hayes, Mariam Bennett, Michael Rhodes, Ayesha Noor, Arjun Mehta, Raghav Sharma';
...
if (!r) {
  ...
  if (!r.releaseRights.registeredReleaseId && !r.registeredReleaseId) {
      r.releaseRights.registeredReleaseId = 'SP-KS-2026-015';
  }
  
  // Update release date per instructions
  r.releaseDate = '2026-02-20';
  r.releaseRights.releaseDate = '2026-02-20';

  fs.writeFileSync(DATA_FILE, JSON.stringify(releases, null, 2), 'utf8');
  console.log('RELEASE FOUND AND PATCHED');
}
console.log('Vocalists Added: ' + addedVoc + ', Updated: ' + updatedVoc);
