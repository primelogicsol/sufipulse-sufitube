...
import { sendVocalistStatusUpdateEmail } from '@/app/lib/email';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = entityUpdate('vocalists', id, { 
    ...
    if (status === 'approved') {
      ...
      if (u) {
        ...
        auditLog({ 
          ...
          resourceType: 'user', 
          resourceId: u.id, 
          details: { role: 'vocalist', profileId: id, adminNote } 
        });
      }
    }
    ...
    if (item.email) {
      (async () => {
        try {
          await sendVocalistStatusUpdateEmail(item.email, status, {
            name: item.performance_name || item.full_name || 'Vocalist',
