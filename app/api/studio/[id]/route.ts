...
import { sendStudioStatusUpdateEmail } from '@/app/lib/email';
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
      ...
      if (item.user_id) {
        ...
        if (u) {
          ...
          auditLog({ 
            ...
            details: { role: 'studio', profileId: id, adminNote: body.admin_note } 
          });
        }
      }
    }
    ...
    // Notify Submitter
    if (item.email) {
      sendStudioStatusUpdateEmail(item.email, status, {
        name: item.studio_name || item.primary_contact_name || 'Studio Partner',
