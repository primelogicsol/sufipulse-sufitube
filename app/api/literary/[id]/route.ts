    ...
    if (status === 'approved' || status === 'rejected') {
      ...
      if (status === 'approved' && item.user_id) {
        ...
        if (u) {
          ...
          auditLog({ userId: authResult.id, userEmail: authResult.email, action: 'role_assigned', resourceType: 'user', resourceId: item.user_id, details: { role: 'literary', profileId: id, adminNote: body.admin_note } });
        }
      }
      auditLog({ userId: authResult.id, userEmail: authResult.email, action: status === 'approved' ? 'profile_approved' : 'profile_rejected', resourceType: 'literary', resourceId: id, details: { adminNote: body.admin_note } });
      if (item.email) {
        notifySubmitterStatusChange({
          to: item.email,
          name: item.professional_name || item.full_name || item.email,
          type: 'literary contributor application',
          status,
