...
import { CheckCircle2, RefreshCw, Youtube } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto p-6 space-y-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-red-400 text-sm font-medium mb-2">
              <Youtube className="w-4 h-4" /> YouTube Release Detection
            </div>
            <h1 className="text-2xl font-bold text-white">Pending Release Approvals</h1>
            <p className="text-neutral-400 mt-2 max-w-3xl">
              New YouTube uploads are detected automatically. Nothing becomes a canonical SufiPulse release until you approve it here. Approval imports the current YouTube title, description, thumbnail and available video metadata. Everything else can be polished afterward in CMS Releases or inline on the public release page.
            </p>
          </div>
          <button
            onClick={load}
            disabled={loading}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-white/10 bg-white/5 text-white hover:bg-white/10 disabled:opacity-50"
          >
        ...
        {loading && items.length === 0 ? (
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.02] p-8 text-center">
            ...
            <p className="text-neutral-500 text-sm mt-2">The six-hour YouTube scan will place any new upload here automatically.</p>
          </div>
        ) : (
          <div className="grid gap-4">
            {items.map((item) => (
              <div key={item.youtubeId} className="rounded-xl border border-white/10 bg-[#111] p-4 flex flex-col md:flex-row gap-4">
                <div className="relative w-full md:w-56 aspect-video rounded-lg overflow-hidden bg-black flex-none">
                  {item.thumbnailUrl ? (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-neutral-500 mt-1">
                    ...
                    {item.publishedDate ? ` · Published ${new Date(item.publishedDate).toLocaleString()}` : ''}
                  ...
                  <div className="flex flex-wrap gap-3 mt-4">
                    <button
                      onClick={() => approve(item.youtubeId)}
                      disabled={approving === item.youtubeId}
                      className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold disabled:opacity-50"
                    >
                      {approving === item.youtubeId ? 'Approving…' : 'Approve as Release'}
                    </button>
                    <a
                      href={`https://www.youtube.com/watch?v=${item.youtubeId}`}
                      target="_blank"
                      rel="noreferrer"
                      className="px-4 py-2 rounded-lg border border-white/10 text-neutral-300 hover:text-white hover:bg-white/5 text-sm"
                    >
                      View on YouTube
                    </a>
                  </div>
        ...
        <div className="pt-2 text-sm text-neutral-500">
          After approval: <Link href="/admin/cms-releases" className="text-neutral-300 hover:text-white underline">open CMS Releases</Link> to refine credits, lyrics, commentary, translations, taxonomy and other governed fields.
        </div>
      </div>
    </DashboardLayout>
  );
