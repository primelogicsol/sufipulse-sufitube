...
export function CinematicHero({
 ...
 return (
 <section
 className={`relative w-full overflow-hidden bg-[var(--color-midnight)] border-b border-[var(--color-border)] hero-bleed ${minHeight.includes('vh') ? '' : minHeight}`}
 style={{ minHeight: minHeight.includes('vh') ? minHeight.replace('min-h-', '').replace('[', '').replace(']', '') : undefined }}
 >
 ...
 priority={priority}
 quality={90}
 className={`object-cover ${backgroundPosition} scale-105 transform motion-safe:animate-fade-in opacity-60`}
 />
 ...
 <div className="max-w-7xl 2xl:max-w-[100rem] mx-auto px-4 sm:px-6 lg:px-8 w-full">
 <div className={`max-w-5xl 2xl:max-w-7xl ${alignClass}`} style={{ paddingTop: 'var(--hero-content-top)' }}>
 ...
 <h1 className="font-serif text-[var(--text-hero)] font-bold text-[var(--color-text-primary)] leading-[1.1] tracking-tight drop-shadow-lg mb-6">
 {title}
 {titleHighlight && (
 <span className="text-[var(--color-gold)] block mt-1 drop-shadow-md">
 {titleHighlight}
 </span>
 )}
 ...
 <p className="text-base sm:text-lg md:text-xl 2xl:text-2xl text-[var(--color-text-secondary)] leading-relaxed font-light max-w-3xl 2xl:max-w-5xl mx-auto mb-10 drop-shadow">
 {subtitle}
 </p>
