...
interface SessionRequestSuccessModalProps {
  onClose: () => void;
  sessionType: 'in_person' | 'remote';
  requestId?: string;
}

export function SessionRequestSuccessModal({ onClose, sessionType, requestId }: SessionRequestSuccessModalProps) {
      ...
      <div className="modal-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-3xl font-semibold text-[var(--color-text-primary)] mb-2 tracking-tight">
            Coordination Request Submitted
          </h2>

          <p className="text-[#D4AF37] text-sm font-medium tracking-wider uppercase mb-6">
            Majalis-e-Sabt — {isInPerson ? 'In-Person Session' : 'Remote Coordination'}
          </p>

          {requestId && (
            <div className="inline-block px-4 py-2 bg-[#0B1B33] border border-[#D4AF37]/20 rounded-lg mb-6">
              <p className="text-xs text-[var(--color-text-tertiary)] mb-1">Request Reference</p>
              <p className="text-sm font-mono text-[#D4AF37] tracking-wide">{requestId}</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4 mb-8">
          <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl">
            <p className="text-[var(--color-text-secondary)] leading-relaxed">
              Your {isInPerson ? 'in-person studio session' : 'remote recording coordination'} request has been formally received by the SufiPulse production board for governance review.
            </p>
          </div>

          <div className="p-5 bg-white/[0.02] border border-white/10 rounded-xl">
            <p className="text-[var(--color-text-secondary)] leading-relaxed">
              Session dates are not confirmed until institutional alignment is completed. No session is scheduled until governance and production approval are granted.
            </p>
          </div>
          ...
          <div className="p-5 bg-linear-to-r from-[#D4AF37]/10 to-transparent border border-[#D4AF37]/20 rounded-xl">
            ...
            <p className="text-[var(--color-text-primary)] font-medium">3–5 Working Days</p>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-3 text-sm text-[var(--color-text-secondary)]">
            <li className="flex items-start gap-3">
              ...
              <span>Governance review of your coordination request and approval reference</span>
            </li>
            <li className="flex items-start gap-3">
              ...
              <span>Alignment with the active production workflow and studio availability</span>
            </li>
            <li className="flex items-start gap-3">
              ...
              <span>Confirmed session date and briefing package issued upon approval</span>
            </li>
            <li className="flex items-start gap-3">
              ...
              <span className="flex items-center gap-2">
                Email notification will be sent with confirmed session details
