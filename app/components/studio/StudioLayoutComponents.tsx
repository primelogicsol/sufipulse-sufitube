      ...
      <PageContainer>
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="max-w-3xl mx-auto">
            <p className="text-[var(--text-lg)] text-[var(--color-text-secondary)] leading-[var(--leading-relaxed)] font-light mb-6 text-center">
              {description}
            </p>
            {disclaimer && (
...
export function StudioSectionHeader({ title, subtitle, badge, centered = false }: StudioSectionHeaderProps) {
  return (
    <div className={`mb-12 ${centered ? 'text-center' : 'text-center md:text-left'}`}>
      ...
      {subtitle && (
        <p className={`text-[var(--color-text-secondary)] ${centered ? 'max-w-3xl mx-auto' : 'max-w-2xl'}`}>
          {subtitle}
        </p>
      )}
    </div>
      ...
      {footerTags && footerTags.length > 0 && (
        <div className="flex flex-wrap gap-2 pt-4 mt-4 border-t border-white/5">
          {footerTags.map((tag, idx) => (
            <span key={idx} className="text-[10px] text-amber-400/70 bg-amber-400/5 px-2 py-0.5 rounded border border-amber-400/10 uppercase font-black tracking-tighter">
              {tag}
            </span>
          ))}
        </div>
...
export function StudioGovernancePanel({ 
  title, 
  description, 
  primaryCTA, 
  secondaryCTA, 
      ...
      <PageContainer>
        <div className="max-w-6xl mx-auto">
          <div className="mb-12 text-center">
            ...
            {description && (
              <p className="text-[var(--color-text-secondary)] mt-4 max-w-3xl mx-auto">
                {description}
              </p>
            )}
          </div>
