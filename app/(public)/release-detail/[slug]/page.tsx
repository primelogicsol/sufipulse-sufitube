...
const VideoOverlay = dynamic(
  () =>
    import("../../../components/release/lyrics/VideoOverlay").then((m) => ({
      default: m.VideoOverlay,
    })),
  { ssr: false },
);
const AdoptTab = dynamic(
  () =>
    import("../../../components/release/adopt/AdoptTab").then((m) => ({
      default: m.AdoptTab,
    })),
  {
    ssr: false,
...
const buildCaptionTrackFromPlainLyrics = (
  release: any,
  selected: string,
...
function Release() {
  ...
  const [selectedSubtitleLanguage, setSelectedSubtitleLanguage] =
    useState<string>("roman_urdu");
  const [selectedLyricsLanguage, setSelectedLyricsLanguage] =
    useState<LanguageKey>("roman_urdu");
  ...
  const handlePlatformStatusChange = async (idx: number, newStatus: string) => {
    ...
    try {
      await fetch(`/api/releases/${release.id}`, {
        ...
        body: JSON.stringify({
          ...release,
          streamingPlatforms: currentPlatforms,
        }),
      });
    } catch (e) {
  ...
  // Lyrics Request Modal State
  const [isLyricsRequestModalOpen, setIsLyricsRequestModalOpen] =
    useState(false);
  ...
  const publicCredits = useMemo(
    () => release?.public_credits || {},
    [release?.public_credits],
  );
  const commentaryBlocks = useMemo(
    () =>
      Array.isArray(release?.public_commentary) &&
      release.public_commentary.length > 0
        ? release.public_commentary.filter(
            (block: any) => block?.isPublished !== false,
          )
        : [
            {
              id: "context",
  ...
  const sponsorsList = useMemo(
    () =>
      Array.isArray(release?.public_sponsors)
        ? release.public_sponsors.filter(
            (sponsor: any) => sponsor?.isPublished !== false,
          )
        : [],
  ...
  const chorusVocalistsLabel = useMemo(() => {
    const chorusVocalists = (release?.chorus_vocalists || []) as Array<
      string | { name?: string }
    >;
  ...
  const envChannelId =
    process.env.NEXT_PUBLIC_YOUTUBE_CHANNEL_ID || "UCraDr3i5A3k0j7typ6tOOsQ";
  const envChannelUrl =
    process.env.NEXT_PUBLIC_YOUTUBE_CHANNEL_URL ||
    `https://www.youtube.com/channel/${envChannelId}`;

  const effectiveYouTubeChannelId =
    releaseChannelId ||
    extractYouTubeChannelIdFromUrl(releaseChannelUrl) ||
    envChannelId;
  const effectiveYouTubeChannelUrl =
    releaseChannelUrl ||
    envChannelUrl ||
    `https://www.youtube.com/channel/${effectiveYouTubeChannelId}`;
  ...
  useEffect(() => {
    ...
    if (
      params.get("adopt") === "1" ||
      params.get("adoption_oauth") === "success"
    ) {
      setActiveTab("adopt");
  ...
  useEffect(() => {
    const existingScript = document.querySelector(
      'script[data-ytsubscribe-script="1"]',
    ...
    const fetchVideoDetails = async () => {
      try {
        const mapCmsRelease = (cmsRelease: any) => ({
          ...
          chorus_vocalists:
            cmsRelease.chorusVocalists || cmsRelease.chorus_vocalists || [],
          production_credits: cmsRelease.producer
            ? { producer: cmsRelease.producer }
            : {},
        ...
        const { youtubeService } = await import(
          "../../../../lib/youtube-service"
        );
  ...
  const effectiveSubtitleTranslations =
    editSubtitleTranslations && Object.keys(editSubtitleTranslations).length > 0
      ? editSubtitleTranslations
      : release?.subtitle_translations || {};

  const effectiveSubtitleCues =
  ...
  const captionLanguageOptions = useMemo(() => {
    ...
    effectiveSubtitleTranslations,
    release?.lyrics,
    release?.lyrics_structure,
  ]);
  ...
  useEffect(() => {
    if (process.env.NODE_ENV === "development") {
      console.log("CC Debug:", {
        ...
        hasSubtitleTranslations:
          !!release?.subtitle_translations?.[selectedCaptionLanguage],
  ...
  useEffect(() => {
    ...
    if (
      !captionLanguageOptions.some(
        (option) => option.key === selectedCaptionLanguage,
      )
    ) {
  ...
  useEffect(() => {
    if (isEditing) {
      ...
      setEditLyrics(
        (release?.lyrics && release?.lyrics[selectedLyricsLanguage]) || [],
      );
      const blocks = Array.isArray(release?.public_commentary)
        ? release.public_commentary
        : [];
      setEditCommentary(
      ...
      setEditSponsors(
        Array.isArray(release?.public_sponsors) ? release.public_sponsors : [],
      );
  ...
  const handleLyricChange = (idx: number, value: string) => {
    setEditLyrics((prev: any[]) =>
      prev.map((line, i) => (i === idx ? value : line)),
    );
  };
  ...
  const handleSubtitleTranslationsChange = (
    language: string,
  ...
  const handleRemoveLyricsBlock = (language: string, blockIdx: number) => {
    setEditStructuredLyrics((prev: any) => {
      ...
      return {
        ...prev,
        [language]: langBlocks.filter((_: any, i: number) => i !== blockIdx),
      };
    });
  };
  const handleLyricsBlockChange = (
    language: string,
    blockIdx: number,
    field: string,
    value: any,
      ...
      return {
        ...
        [language]: langBlocks.map((b: any, i: number) =>
          i === blockIdx
            ? {
                ...b,
                lines: b.lines.map((l: string, j: number) =>
                  j === lineIdx ? value : l,
                ),
              }
            : b,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setRelease((prev: any) => ({
        ...
        duration_seconds:
          masterTimingDuration || release?.duration_seconds || 0,
        availableLanguages: editAvailableLanguages,
        defaultLanguage: editDefaultLanguage,
        subtitleLanguageStatuses: editLanguageStatuses,
      ...
      <Layout>
        <PageContainer>
          <div className="max-w-7xl mx-auto animate-pulse">
            ...
            {/* Video placeholder */}
            <div
              className="relative w-full bg-neutral-900 border border-neutral-800 rounded-2xl overflow-hidden mb-8"
            ...
            <div className="flex gap-2 mb-8">
              {[80, 64, 72, 60, 68].map((w, i) => (
                <div
                  key={i}
                  className="h-9 rounded-full bg-neutral-800"
                  style={{ width: w }}
                />
              ))}
            </div>
          ...
          {lines.map((line, lineIndex) => {
            if (line.match(/^\d{2}:\d{2}/)) {
              return (
                <div
                  key={lineIndex}
                  className="text-neutral-400 text-base mb-1.5 font-medium"
                >
                  {line}
                </div>
              );
            ...
            if (
              ...
              line.includes("Theme:") ||
              line.includes("Produced under")
            ) {
              return (
                <div
                  key={lineIndex}
                  className="text-neutral-400 text-base mb-1.5 flex items-center gap-2"
                >
            ...
            return (
              <p
                key={lineIndex}
                className="text-neutral-300 text-base leading-relaxed mb-3"
              >
                {line}
              </p>
            );
  ...
  const getAvailableLanguages = () => {
    const legacy = release.lyrics
      ? Object.keys(release.lyrics).filter((key) => release.lyrics[key])
      : [];
    const structured = Object.keys(lyricsStructureMap || {}).filter((key) =>
      Array.isArray(lyricsStructureMap[key]),
    );
  ...
  const exportVttFile = () => {
    ...
    if (!cues.length) {
      setToastType("info");
      setToastMessage(
        "No timestamp cues found to export. Please generate master timing first.",
      );
      return;
    }
  ...
  return (
    <Layout>
      <PageContainer>
        <div className="max-w-7xl mx-auto">
          <div
            className="sr-only"
            aria-hidden="true"
            data-resolution-source={resolutionSource || "unknown"}
          >
            ...
            <div className="flex justify-end mb-4 gap-3">
              {!isEditing ? (
                ...
                <div className="text-sm text-amber-400 flex items-center gap-2">
                  <Edit className="w-4 h-4" />
                  <span>
                    Inline editing mode active - use the save bar at the bottom
                  </span>
                </div>
              )}
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs text-neutral-400">
                    ...
                    <div>
                      <span className="text-neutral-300 font-medium">
                        Advanced editing (use Full CMS Editor):
                      </span>
                      <ul className="mt-1 space-y-0.5">
                  ...
                  {!videoLoaded ? (
                    <>
                      ...
                      <div className="absolute inset-0 bg-black/40 group-hover:bg-black/30 transition-colors flex items-center justify-center">
                        ...
                        >
                          <Play
                            className="w-10 h-10 text-red-600 ml-1.5"
                            fill="currentColor"
                          />
                        </button>
                      </div>
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-lg font-semibold text-neutral-100">
                      Continue the SufiPulse Journey
                    </h3>
                    <p className="mt-1 text-sm text-neutral-400">
                      Listen to another release, subscribe on YouTube, or join
                      SufiPulse.
                    </p>
                  </div>
              ...
              <div className="flex items-center gap-2 sm:gap-3 lg:pb-2 flex-shrink-0">
                ...
                {resolvedVideoId && (
                  <a
                    ...
                    rel="noopener noreferrer"
                    onClick={() =>
                      trackEvent("CTA_WatchOnYouTube", {
                        context: "player",
                        videoId: resolvedVideoId || "",
                      })
                    }
                    title={
                      captionsEnabled && hasReached70
                        ? "Full-quality captions available on YouTube"
                        : undefined
                    }
                    className={`inline-flex items-center gap-2 px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-all shadow-lg shadow-red-900/30 whitespace-nowrap${hasReached70 ? " ring-1 ring-amber-400/40" : ""}`}
                  >
          ...
          <div className="mb-16">
                    ...
                    {(() => {
                      ...
                      > = {
                        production: {
                          ...
                          recordingEngineer:
                            'Lucas Ray, Michael "SufiPulse" Hartman',
                      ...
                      const renderSeededSection = (
                        ...
                        <div
                          className={`p-6 bg-gradient-to-br from-neutral-900 to-neutral-900/50 border border-neutral-800 rounded-xl ${sectionKey === "rights" ? "md:col-span-2" : ""}`}
                        >
                          <div className="flex items-center gap-3 mb-6">
                            <div className="w-10 h-10 rounded-full bg-amber-400/10 flex items-center justify-center">
                              {icon}
                        ...
                        <>
                          ...
                          {renderSeededSection(
                            ...
                            [
                              ...
                              {
                                label: "Registered Release ID",
                                key: "registeredReleaseId",
                              },
                ...
                <div className="bg-neutral-900/50 border border-neutral-800 rounded-lg p-8">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-xl font-medium text-neutral-100">
                      Subtitles & Lyrics Languages
                    </h3>
                    {isEditing && (
                      <span className="text-xs text-amber-400 bg-amber-900/20 border border-amber-800/40 px-3 py-1 rounded">
                        ✏️ Step 1: Manage Languages → Step 2: Create Master Cues
                      ...
                      <div className="bg-neutral-800 border border-amber-800/40 rounded-lg p-6 mb-8">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h4 className="text-lg font-medium text-neutral-100">
                            Language Management (Up to 17 Languages)
                          </h4>
                        </div>
                        <p className="text-sm text-neutral-500 mb-4">
                          Select which languages to enable for this release. One
                          language must be set as the master (with timing).
                        </p>

                        {/* Master Language Selector */}
                        <div className="flex items-center gap-4 mb-6">
                          <label className="text-sm text-neutral-400">
                            Master Language (with timing):
                          </label>
                          <select
                          ...
                          <span className="text-xs text-amber-400">
                            ✓ This language's lyrics will have timestamps -
                            other languages sync to it
                          </span>
                        </div>
                        ...
                        <div>
                          <p className="text-sm text-neutral-500 mb-3">
                            Available Languages (check to enable):
                          </p>
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                            {PAGE_LANGUAGE_OPTIONS.map((lang) => {
                              const isActive = editAvailableLanguages.includes(
                                lang.code,
                              );
                              const isMaster =
                                lang.code === editDefaultLanguage;
                              const isCcDisplay =
                                lang.code === selectedCaptionLanguage;
                              const status =
                                editLanguageStatuses?.[lang.code] || "draft";
                              const hasSubtitles =
                                editSubtitleTranslations?.[lang.code] &&
                                Object.keys(
                                  editSubtitleTranslations[lang.code] || {},
                                ).length > 0;
                              const hasLyrics =
                                release?.lyrics?.[lang.code] ||
                                editStructuredLyrics?.[lang.code];
                                ...
                                >
                                      ...
                                      onChange={() => {
                                        ...
                                        if (current.includes(lang.code)) {
                                          ...
                                          const next = current.filter(
                                            (c: string) => c !== lang.code,
                                          );
                                          // If removing master, pick new master
                                          if (
                                            lang.code === editDefaultLanguage
                                          ) {
                                            setEditDefaultLanguage(
                                              next[0] || "",
                                            );
                                          }
                                  ...
                                  {isActive && (
                                    <select
                                      value={status}
                                      onChange={(e) =>
                                        handleLanguageStatusChange(
                                          lang.code,
                                          e.target.value as SubtitleStatus,
                                        )
                                      }
                    ...
                    <div className="space-y-8">
                      ...
                      <div className="bg-neutral-800 border border-amber-800/40 rounded-lg p-6">
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <h4 className="text-lg font-medium text-neutral-100">
                            Master Subtitle Cues (Timing)
                          </h4>
                        </div>
                        <p className="text-sm text-neutral-500 mb-4">
                          Paste the full song — one line per subtitle cue. Set
                          the video duration, and timing will auto-generate.
                        </p>
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          <span className="text-xs text-neutral-600">
                            {release.duration_seconds
                              ? `${Math.floor(release.duration_seconds / 60)}:${(release.duration_seconds % 60).toString().padStart(2, "0")} mm:ss`
                              : "Set this first"}
                          </span>
                        </div>
                        ...
                        <textarea
                          ...
                          value={(() => {
                            ...
                            return cues
                              .sort(
                                (a, b) =>
                                  (a.cueNumber || 0) - (b.cueNumber || 0),
                              )
                              .map(
                                (cue: any) => masterTranslations[cue.id] || "",
                              )
                              .filter(Boolean)
                              .join("\n");
                          })()}
                          onChange={(e) => {
                            ...
                            const cueDuration =
                              lines.length > 0 ? duration / lines.length : 3;

                            const newCues = lines.map(
                          ...
                          <div className="mt-4">
                            <div className="flex items-center justify-between mb-2">
                              <span className="text-sm text-neutral-400">
                                ...
                                {editSubtitleTranslations[
                                  ...
                                  ` • ${Object.keys(editSubtitleTranslations[editDefaultLanguage]).length} translations`}
                              </span>
                            </div>
                            <div className="max-h-60 overflow-y-auto bg-neutral-950 border border-neutral-800 rounded p-3">
                              {editSubtitleCues
                                ...
                                .map((cue: any) => {
                                  ...
                                  return (
                                    <div
                                      key={cue.id}
                                      className="flex items-start gap-3 py-1.5 border-b border-neutral-900 last:border-0 text-xs"
                                    >
                                      <span className="text-neutral-600 font-mono w-8 flex-shrink-0">
                                        #{cue.cueNumber}
                                      </span>
                                      <span className="text-neutral-500 font-mono w-28 flex-shrink-0">
                                        {cue.startTime}
                                      </span>
                              ...
                              {editSubtitleCues.length > 50 && (
                                <div className="text-neutral-600 text-xs italic py-1">
                                  ... and {editSubtitleCues.length - 50} more
                                  cues
                                </div>
                              )}
                            </div>
                      ...
                      {editSubtitleCues.length > 0 && (
                        <div className="bg-neutral-800 border border-blue-800/40 rounded-lg p-6">
                          <div className="flex items-center gap-3 mb-4">
                            <div className="w-8 h-8 rounded-full bg-blue-400/10 flex items-center justify-center text-blue-400 text-sm font-bold">
                              2
                            </div>
                            <h4 className="text-lg font-medium text-neutral-100">
                              Translations (each language follows master timing)
                            </h4>
                          </div>
                          <p className="text-sm text-neutral-500 mb-4">
                            Select a language, paste the translated text (one
                            line per cue). Timing is inherited from master cues.
                          </p>
                                  ...
                                  >
                                    ...
                                    {transCount > 0 && (
                                      <span className="ml-1 text-blue-400">
                                        ({transCount}/{editSubtitleCues.length})
                                      </span>
                                    )}
                                  </button>
                                );
                          ...
                          {selectedSubtitleLanguage &&
                            selectedSubtitleLanguage !== editDefaultLanguage &&
                            (() => {
                              const langTranslations =
                                editSubtitleTranslations?.[
                                  selectedSubtitleLanguage
                                ] || {};
                              const sortedCues = [...editSubtitleCues].sort(
                                (a, b) =>
                                  (a.cueNumber || 0) - (b.cueNumber || 0),
                              );
                              const existingText = sortedCues
                                .map(
                                  (cue: any) => langTranslations[cue.id] || "",
                                )
                                .filter(Boolean)
                                .join("\n");
                                      ...
                                      <button
                                        onClick={() => {
                                          ...
                                          sortedCues.forEach((cue: any) => {
                                            newTranslations[cue.id] =
                                              masterTranslations[cue.id] || "";
                                          });
                                          setEditSubtitleTranslations(
                                            (prev: any) => ({
                                              ...prev,
                                              [selectedSubtitleLanguage]:
                                                newTranslations,
                                          ...
                                          setToastType("info");
                                          setToastMessage(
                                            `Copied ${editDefaultLanguage} as base for ${getLanguageLabel(selectedSubtitleLanguage)}. Now replace with actual translation.`,
                                          );
                                          setTimeout(
                                  ...
                                  <div className="mt-4 max-h-60 overflow-y-auto bg-neutral-950 border border-neutral-800 rounded p-3">
                                    {sortedCues
                                      ...
                                      .map((cue: any, idx: number) => {
                                        ...
                                        const transText =
                                          langTranslations[cue.id] || "";
                                        return (
                                          <div
                                            key={cue.id}
                                            className={`flex items-start gap-3 py-1.5 border-b border-neutral-900 last:border-0 text-xs ${transText ? "" : "opacity-40"}`}
                                          >
                                            <span className="text-neutral-600 font-mono w-8 flex-shrink-0">
                                              #{cue.cueNumber}
                                            </span>
                                    ...
                                    {editSubtitleCues.length > 30 && (
                                      <div className="text-neutral-600 text-xs italic py-1">
                                        ... and {editSubtitleCues.length - 30}{" "}
                                        more cues
                                      </div>
                                    )}
                                  </div>
                      ...
                      {editAvailableLanguages.length > 0 ? (
                        ...
                        <div className="text-neutral-500 text-center py-12">
                          ...
                          <p className="text-sm mt-2">
                            Click "Edit Release" to add subtitle cues and
                            translations.
                          </p>
                        </div>
                      )}
              ...
              <div className="pt-8">
                <div className="bg-neutral-900/50 border border-neutral-800 rounded-lg p-8">
                  <div className="flex items-center justify-between mb-6">
                    ...
                    {isEditing && (
                      <span className="text-xs text-amber-400 bg-amber-900/20 border border-amber-800/40 px-3 py-1 rounded">
                        ✏️ Master language has timing - translations sync
                        automatically
                      </span>
                    )}
                  </div>
                        ...
                        <div className="p-6 bg-neutral-900 border border-amber-800/40 rounded-xl shadow-2xl mt-6 flex flex-col max-h-[600px]">
                          <div className="flex items-center justify-between mb-4 shrink-0">
                            <div>
                              <h4 className="text-lg font-medium text-amber-400">
                                Interactive Video Sync
                              </h4>
                              <p className="text-xs text-neutral-500">
                                Play the video and click to capture timestamps
                                live.
                              </p>
                            </div>
                          ...
                          <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1 pb-4">
                            {editSubtitleCues.map((cue, idx) => (
                              <div
                                key={cue.id}
                                className="flex flex-col gap-3 p-4 bg-black/40 border border-neutral-800 rounded-lg group hover:border-neutral-700 transition-colors"
                              >
                                      ...
                                      <div className="flex items-stretch bg-black border border-neutral-700 rounded overflow-hidden focus-within:border-amber-500 transition-colors">
                                        <button
                                          onClick={() => {
                                            ...
                                            const ts =
                                              formatSecondsToTimestamp(secs);
                                            setEditSubtitleCues((prev) =>
                                              prev.map((c) =>
                                                c.id === cue.id
                                                  ? { ...c, startTime: ts }
                                                  : c,
                      ...
                      <div className="p-6 bg-neutral-900 border border-amber-800/40 rounded-xl shadow-2xl">
                        <div className="flex items-center gap-3 mb-6">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-neutral-500">
                              Create the primary timing structure for
                              synchronization.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4">
                          <div>
                            ...
                            <div className="flex items-center gap-4">
                              <input
                                type="number"
                                value={
                                  masterTimingDuration ||
                                  release?.duration_seconds ||
                                  0
                                }
                                onChange={(e) =>
                                  setMasterTimingDuration(
                          ...
                          <div>
                            <label className="block text-sm font-medium text-neutral-400 mb-2">
                              Paste Master Lyrics (one line per cue):
                            </label>
                            <textarea
                              value={masterTimingText}
                              onChange={(e) =>
                                setMasterTimingText(e.target.value)
                              }
                        ...
                        <div className="p-4 bg-amber-900/10 border border-amber-800/30 rounded-lg flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            <p className="text-sm text-amber-300">
                              ...
                              <strong className="text-neutral-100">
                                {getLanguageLabel(
                                  editDefaultLanguage || "roman_urdu",
                                )}
                              </strong>
                              .
                            </p>
                            <p className="text-xs text-neutral-500 mt-1">
                              Switch to any other language below to paste
                              translations. They will automatically map to these
                              timestamps.
                            </p>
                          </div>
                        </div>
                  ...
                  {captionLanguageOptions.length > 0 ? (
                    <>
                      <div className="flex flex-wrap items-center gap-3 mb-8">
                        {(() => {
                          ...
                          const primaryOptions = availableOptions
                            ...
                            .sort(
                              (a, b) =>
                                priorityKeys.indexOf(a.key) -
                                priorityKeys.indexOf(b.key),
                            );

                          const otherOptions = availableOptions.filter(
                            (opt) => !priorityKeys.includes(opt.key),
                          );

                          const isOtherSelected = otherOptions.some(
                            (opt) => opt.key === selectedLyricsLanguage,
                          );
                          ...
                          return (
                            <>
                              ...
                              {primaryOptions.map((opt) => {
                                const isSelected =
                                  selectedLyricsLanguage === opt.key;
                                return (
                                  <button
                                    key={opt.key}
                                    onClick={() => {
                                      setSelectedLyricsLanguage(
                                        opt.key as LanguageKey,
                                      );
                                  ...
                                  >
                                    ...
                                    {isEditing && (
                                      <span
                                        className={`absolute top-1 right-1.5 text-[9px] uppercase tracking-wider px-1.5 py-0.5 rounded ${
                                          opt.status === "verified"
                      ...
                      {selectedLyricsLanguage &&
                        (() => {
                          ...
                          const structuredBlocks = Array.isArray(
                            lyricsStructureMap?.[activeLanguage],
                          )
                            ? lyricsStructureMap[activeLanguage].filter(
                                (block: any) => block?.isPublished !== false,
                              )
                            : [];
                          const legacyRows = Array.isArray(
                            release.lyrics?.[activeLanguage],
                          )
                            ? release.lyrics[activeLanguage]
                            : [];

                          // Support fetching cleanly from editSubtitleTranslations to surface ghost data
                          const hiddenTranslations = editSubtitleTranslations?.[
                            activeLanguage
                          ]
                            ? editSubtitleCues && editSubtitleCues.length > 0
                              ? editSubtitleCues
                                  .map(
                                    (c: any) =>
                                      editSubtitleTranslations[activeLanguage][
                                        c.id
                                      ],
                                  )
                                  .filter(Boolean)
                          ...
                          return isEditing ? (
                            <div className="space-y-4">
                              <div className="flex items-center justify-between">
                                <h4 className="text-lg font-medium text-amber-400">
                                  Editing:{" "}
                                  {getLanguageLabel(selectedLyricsLanguage)}
                                </h4>
                                <button
                                  type="button"
                                  onClick={() => {
                                    ...
                                    setRelease((prev: any) => ({
                                      ...
                                      lyrics_structure: {
                                        ...(prev?.lyrics_structure || {}),
                                        [active]: [],
                                      },
                                      lyricsStructure: {
                                        ...(prev?.lyricsStructure || {}),
                                        [active]: [],
                                      },
                                      subtitle_translations: {
                                        ...(prev?.subtitle_translations || {}),
                                        [active]: {},
                                      },
                            ...
                            <>
                              {flatLyricsText.trim().length > 0 ? (
                                ...
                                <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-10 mt-6 text-center">
                                  <Globe
                                    className="w-12 h-12 text-neutral-700 mx-auto mb-4"
                                    strokeWidth={1.5}
                                  />
                                  <p className="text-neutral-400 mb-6">
                                    Lyrics are not available yet in{" "}
                                  ...
                                  <button
                                    onClick={() =>
                                      setIsLyricsRequestModalOpen(true)
                                    }
                                    className="inline-flex items-center gap-2 px-6 py-2.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-lg transition-all text-sm font-medium"
                                  >
                    ...
                    <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-10 text-center">
                      <Globe
                        className="w-12 h-12 text-neutral-700 mx-auto mb-4"
                        strokeWidth={1.5}
                      />
                      ...
                      <button
                        onClick={() => setIsLyricsRequestModalOpen(true)}
                        className="inline-flex items-center gap-2 px-6 py-2.5 bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 rounded-lg transition-all text-sm font-medium"
                      >
                        <Send className="w-4 h-4" />
            ...
            {activeTab === "production" && release.production_credits && (
              <div className="pt-8">
                <div className="bg-neutral-900/50 border border-neutral-800 rounded-lg p-8">
                  <h3 className="text-2xl font-medium text-neutral-100 mb-8">
                    ...
                    <div className="mb-8 p-6 bg-neutral-900 border border-neutral-800 rounded-lg">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-xl text-neutral-300 font-medium mb-3">
                            {release.production_credits.studio_name}
                          </p>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="space-y-3">
                            ...
                            {release.production_credits.engineering_crew &&
                              release.production_credits.engineering_crew
                                .length > 0 && (
                                <div className="mt-4 pt-4 border-t border-neutral-800">
                                  ...
                                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {release.production_credits.engineering_crew.map(
                                      (member: any, idx: any) => (
                                        <div
                                          key={idx}
                                          className="flex justify-between items-center bg-neutral-950 rounded px-4 py-2"
                                        >
                  ...
                  {release.production_credits.instrumentalists &&
                    release.production_credits.instrumentalists.length > 0 && (
                      <div className="mb-8 p-6 bg-neutral-900 border border-neutral-800 rounded-lg">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                              {release.production_credits.instrumentalists.map(
                                (musician: any, idx: any) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between items-center bg-neutral-950 rounded px-4 py-3"
                                  >
                                    <span className="text-neutral-300">
                                      {musician.name}
                                    </span>
                      ...
                      <div className="space-y-6">
                        <div className="bg-neutral-800 border border-amber-800/30 rounded-xl p-6">
                          <textarea
                            ...
                            onChange={(e) =>
                              handleCommentaryChange(e.target.value)
                            }
            ...
            {activeTab === "sponsors" && release.enable_sponsors && (
              <div className="pt-8">
                <div className="bg-neutral-900/50 border border-neutral-800 rounded-lg p-8">
                  <h3 className="text-3xl font-serif font-light text-neutral-100 mb-8">
                    Our Sponsors
                  </h3>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {sponsorsList.map((sponsor: any, idx: number) => (
                      <div
                        key={sponsor.id || idx}
                        className="bg-neutral-900 border border-neutral-800 rounded-xl p-6 flex flex-col items-center justify-center aspect-square group hover:border-amber-400/30 transition-colors"
                      >
                    ...
                    >
                      <div className="w-12 h-12 rounded-full border border-neutral-700 border-dashed flex items-center justify-center mb-3">
                        <span className="text-neutral-500 text-2xl group-hover:text-amber-400 font-light">
                          +
                        </span>
                      </div>
                      <p className="text-neutral-500 text-sm text-center group-hover:text-amber-400 transition-colors">
                        Become a Sponsor
                      </p>
                    </div>
                  </div>
          ...
          {!isLegacy && (
            <section className="mb-16">
              <h2 className="text-2xl font-serif font-light text-neutral-100 mb-8">
                Streaming Platforms
              </h2>
              <div className="space-y-3">
                {release.streaming_platforms &&
                release.streaming_platforms.length > 0 ? (
                  release.streaming_platforms.map((p: any, idx: number) => {
                    const statusDisplay = isAdmin ? (
                      <select
                        value={p.status === "Available" ? "Live" : p.status}
                        onChange={(e) =>
                          handlePlatformStatusChange(idx, e.target.value)
                        }
                      ...
                      >
                        ...
                        <option value="Distribution Pending">
                      ...
                      <span className="text-sm text-amber-500">
                        {p.status === "Available" ? "Live" : p.status}
                      </span>
                    );
                    return p.url ? (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex items-center justify-between p-4 bg-neutral-900 border border-neutral-800 rounded-lg hover:border-neutral-700 transition-colors cursor-pointer group"
                      >
                        <span className="text-neutral-300 group-hover:text-amber-400 transition-colors">
                          {p.platform}
                        </span>
                        {statusDisplay}
                      </a>
                  ...
                  <>
                    {[
                      {
                        platform: "SufiPulse Radio",
                        status: "Distribution Pending",
                      },
                      { platform: "YouTube", status: "Distribution Pending" },
                      {
                        platform: "Spotify",
                        status: release.spotify_url
                          ? "Live"
                          : "Distribution Pending",
                      },
                      {
                        platform: "Apple Music",
                        status: release.apple_music_url
                          ? "Live"
                          : "Distribution Pending",
                      },
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-neutral-200">
                      Did this kalam move you?
                    </p>
                    <p className="text-xs text-neutral-500 mt-1 leading-relaxed">
                      Leave a comment on YouTube — it helps the algorithm
                      promote this to more listeners worldwide.
                    </p>
                  </div>
                </div>
                <a
      ...
      {toastMessage && (
        <div
          className={`fixed bottom-20 left-1/2 -translate-x-1/2 z-50 px-6 py-3 rounded-lg shadow-2xl border backdrop-blur-md animate-in fade-in slide-in-from-bottom-4 duration-300 ${
            toastType === "success"
...
function SocialShareKitPanel({
  kit,
}: {
  kit: NonNullable<any["socialShareKit"]>;
}) {
  ...
  const generated = kit.generatedAt
    ? new Date(kit.generatedAt).toLocaleDateString("en-GB", {
        day: "numeric",
        month: "short",
        year: "numeric",
      })
  ...
  return (
    <div className="mb-4 rounded-xl border border-amber-900/30 bg-neutral-900/50 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xs uppercase tracking-[0.18em] text-amber-500/70 font-medium">
            Social Share Kit
          </span>
          {generated && (
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-neutral-800 pt-3">
          ...
          <p className="text-xs text-neutral-600 pt-1">
            Auto-generated on publish. Copy and paste to your social channels.
          </p>
        </div>
      )}
    </div>
...
function ContributorShareKit({
  ...
  const vocalistName =
    release?.lead_vocalists?.[0]?.name || release?.vocalist?.name || "";
  const writerName =
    release?.public_credits?.literary?.[0]?.name || release?.writer?.name || "";
    ...
    <div className="mb-4 rounded-xl border border-neutral-800 bg-neutral-900/50 overflow-hidden">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 border-t border-neutral-800 pt-3">
          ...
          <p className="text-xs text-neutral-600 pt-1">
            Send these to your contributors to share on their social channels.
          </p>
        </div>
      )}
    </div>
