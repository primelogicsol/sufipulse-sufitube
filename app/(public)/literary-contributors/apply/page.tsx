...
function LiteraryApplyContent() {
  ...
  useEffect(() => {
    if (ref && token) {
      setLoading(true);
      fetch(`/api/literary/${ref}?token=${token}`)
        .then(res => res.json())
        .then(data => {
          if (!data.error) {
            setInitialData(data);
  ...
  const handleSuccess = (submissionId: string, token: string) => {
    // Redirect to a specific success page if needed, or handle modal logic inside the form
    // For consistency with Writers, we could have a /submitted page, but let's see.
    // The current form shows a modal on success.
  };
  ...
  return (
    <div className="w-full">
      <div className="max-w-4xl mx-auto text-center mb-16">
        <Badge variant="gold" className="mb-6 px-4 py-1.5 text-[10px] font-black tracking-[0.3em]">Ahl-e-Tahreer Intake</Badge>
        <h1 className="text-4xl md:text-6xl font-bold text-white mb-4 tracking-tighter">
          Literary Contributor Pathway
        </h1>
        <p className="text-neutral-500 text-sm md:text-base max-w-2xl mx-auto leading-relaxed">
          Institutional editorial intake for reflective writing, essays, and spiritual discourse seeking publication within the SufiPulse Literary Journal.
        </p>
      </div>
          ...
          <div className="max-w-7xl mx-auto mt-20 pt-10 border-t border-white/5 flex flex-col md:flex-row items-center justify-between gap-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[9px] text-neutral-600 uppercase tracking-widest mt-1">Registry adherence is mandatory for all contributors</p>
              </div>
            </div>
