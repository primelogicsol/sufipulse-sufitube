...
// Send resolutions in POST
content = content.replace(/body: JSON\.stringify\(\{ videoIds: ids \}\),/g, `body: JSON.stringify({ videoIds: ids, resolutions }),`);

// Update UI to show diff and buttons
const uiRegex = /\{video\.cmsReleaseId && \(\s*<a href=\{\`\/admin\/cms-releases\/\$\{video\.cmsReleaseId\}\`\}[\s\S]*?<\/div>\s*\)\s*:\s*\(/g;

const newUi = `{video.cmsReleaseId && (
