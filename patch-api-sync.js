...
const syncLogic = `
  if (url.searchParams.get('runDescSync') === '1') {
...
content = content.replace(/(export async function GET\(request: NextRequest\) \{[\s\S]*?const url = new URL\(request\.url\);)/, `$1\n${syncLogic}`);
